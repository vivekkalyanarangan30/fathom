...
\caption{Decode step versus context with K/V rows and scan index in pinned host memory (A100, Qwen3-8B, $k=512$, batch 1, synthetic KV at every point). (a) wall-clock, (b) GPU time. Every method's index is moved with the contiguous gather of \S\ref{sec:gather}.}
\label{fig:offload}
\end{figure*}
\begin{table*}[t]\centering\small
